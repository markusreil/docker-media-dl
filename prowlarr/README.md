...
ACME companion using `VIRTUAL_HOST` — at `prowlarr.<BASE_DOMAIN>` (see
`BASE_DOMAIN` in `.env`).
...
| `GEN_SELF_SIGNED_CERT` | Self-signed certificate opt-in for the LAN/self-signed proxy variant; wired from `PROWLARR_GEN_SELF_SIGNED_CERT` (default `false`; set `true` for LAN/self-signed). Declared alongside `ACME_HOST` so the proxy variant decides which applies |

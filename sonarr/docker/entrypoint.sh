...
# Idempotent upsert of the Sabnzbd download client via the Sonarr v3 API.
...
seed_sabnzbd_client() {
    base="http://127.0.0.1:8989/api/v3/downloadclient"
    # Wait for the API (up to ~120s).
    i=0
    while [ "$i" -lt 60 ]; do
        if wget -q -O /dev/null --header="X-Api-Key: $SONARR_API_KEY" \
            http://127.0.0.1:8989/api/v3/system/status 2>/dev/null; then
            break
        fi
        i=$((i + 1))
        sleep 2
    ...
    clients=$(wget -q -O - --header="X-Api-Key: $SONARR_API_KEY" "$base" 2>/dev/null || true)
    [ -n "$clients" ] || return 0
    id=$(printf '%s' "$clients" | jq -r '.[] | select(.name=="Sabnzbd") | .id // empty' | head -n 1)

    schema=$(wget -q -O - --header="X-Api-Key: $SONARR_API_KEY" "$base/schema" 2>/dev/null || true)
    [ -n "$schema" ] || return 0
    # tvCategory must be left blank, exactly like Radarr's movieCategory: this
    # Sonarr pin cannot match ANY SABnzbd category by name. Its
    # SabnzbdCategory deserializer maps JSON keys to C# properties verbatim
    # (PascalCase: "Name"/"Dir"), but SABnzbd sends lowercase ("name"/"dir"),
    # so every category comes back with a null Name and a non-blank tvCategory
    # always fails the server-side "Category does not exist" check
    # (Sabnzbd.cs GetCategories FirstOrDefault(v => v.Name == TvCategory),
    # shared Servarr code — same behavior as Radarr's movieCategory). A blank
    # value rides SABnzbd's catch-all "*" and completes at the complete_dir
    # root — the layout SABnzbd seeds (tv dir=""). Sabnzbd.cs GetItems()
    # matches "*" when TvCategory is blank. Follow the pinned-version behavior
    # rather than the docs; revisit when bumping the Sonarr pin.
    body=$(printf '%s' "$schema" | jq \
        ...
        | .fields=(.fields
            | map(if .name=="host" then .value="sabnzbd"
                ...
                elif .name=="tvCategory" then .value=""
                else . end))')
    ...
    while [ "$attempt" -le 10 ]; do
        ...
        if [ -n "$cur_id" ]; then
            ...
            wget -q -O /dev/null --header="X-Api-Key: $SONARR_API_KEY" \
...
# Idempotent upsert of the qBittorrent download client via the Sonarr v3 API.
...
seed_qbittorrent_client() {
    ...
    schema=$(wget -q -O - --header="X-Api-Key: $SONARR_API_KEY" "$base/schema" 2>/dev/null || true)
    [ -n "$schema" ] || return 0
    # tvCategory is explicitly pinned to Sonarr's own default ("tv-sonarr",
    # QBittorrentSettings). It stays distinct from Radarr's "movies" category
    # so the two *arr apps never share a qBittorrent path; qBittorrent
    # auto-creates the category on first use, defaulting the save path to
    # /data/torrents/complete/tv-sonarr. Distinct from Radarr's movies category
    # so completed downloads for each app land in their own subdir.
    body=$(printf '%s' "$schema" | jq \
...
# Idempotent upsert of Sonarr as an Application inside Prowlarr.
# Servarr topology: Sonarr has no native Prowlarr indexer implementation;
# Prowlarr pushes trackers into Sonarr as Newznab/Torznab entries.
# Built from the GET /api/v1/applications/schema template
# (implementation == "Sonarr"), matched by name for the existing id,
...
seed_prowlarr_app() {
    ...
    [ -n "$apps" ] || return 0
    id=$(printf '%s' "$apps" | jq -r '.[] | select(.name=="Sonarr") | .id // empty' | head -n 1)

    schema=$(wget -q -O - --header="X-Api-Key: $PROWLARR_API_KEY" "$base/schema" 2>/dev/null || true)
    [ -n "$schema" ] || return 0
    # syncCategories mirrors Prowlarr's own Sonarr Settings default
    # (SonarrSettings.cs: SyncCategories = 5000,5010,5020,5030,5040,5045,5050,5090).
    body=$(printf '%s' "$schema" | jq \
        --arg apiKey "$SONARR_API_KEY" \
        '.[] | select(.implementation=="Sonarr") | .enable=true
        ...
        | .fields=(.fields
            | map(if .name=="prowlarrUrl" then .value="http://prowlarr:9696"
                ...
                elif .name=="syncCategories" then .value=[5000,5010,5020,5030,5040,5045,5050,5090]
                else . end))')
    [ -n "$body" ] && [ "$body" != "null" ] || return 0

    attempt=1
    while [ "$attempt" -le 10 ]; do
        cur_id=$(printf '%s' "$apps" | jq -r '.[] | select(.name=="Sonarr") | .id // empty' 2>/dev/null | head -n 1 || true)
        if [ -n "$cur_id" ] && [ "$attempt" -gt 1 ]; then
            echo "seed: Sonarr application present (id $cur_id)"
            break
        fi
        if [ -n "$cur_id" ]; then
            req_body=$(printf '%s' "$body" | jq --argjson id "$cur_id" '. + {id: $id, name: "Sonarr"}' || true)
            wget -q -O /dev/null --header="X-Api-Key: $PROWLARR_API_KEY" \
                --header="Content-Type: application/json" \
                --method=PUT --body-data="$req_body" "$base/$cur_id" 2>/dev/null || true
        else
            req_body=$(printf '%s' "$body" | jq '. + {name: "Sonarr"}' || true)
            wget -q -O /dev/null --header="X-Api-Key: $PROWLARR_API_KEY" \
                --header="Content-Type: application/json" \
        ...
        verify_id=""
        if [ -n "$apps" ]; then
            verify_id=$(printf '%s' "$apps" | jq -r '.[] | select(.name=="Sonarr") | .id // empty' 2>/dev/null | head -n 1 || true)
        fi
        if [ -n "$verify_id" ]; then
            echo "seed: Sonarr application present (id $verify_id)"
            break
        fi
        echo "seed: Sonarr upsert attempt $attempt failed, retrying"
        attempt=$((attempt + 1))
        [ "$attempt" -le 10 ] && sleep 10 || true
    done
...
if [ "$(id -u)" = "0" ]; then
    addgroup -g "$PGID" -S sonarr 2>/dev/null || true
    adduser -u "$PUID" -S -G sonarr -h /config -s /sbin/nologin sonarr 2>/dev/null || true

    mkdir -p /config

    # Sonarr has no env-var or CLI override for its API key, so seed a
    ...
    #   it is also what Sonarr's own stock config seeds, and it broke DI in
    #   this build). This matches the stack's LAN-only posture (SABnzbd
    #   behaves the same way). Sonarr deliberately refuses the
    ...
    #   Sonarr crashes with a DryIoc InvalidCastException (500 on
    ...
    # Start Sonarr in the background as the unprivileged user, seed the
    ...
    # then wait on Sonarr as the foreground action.
    su-exec "$PUID:$PGID" /app/Sonarr -nobrowser -data=/config &
    SONARR_PID=$!
    if [ -n "${SONARR_API_KEY:-}" ] && [ -n "${SABNZBD_API_KEY:-}" ]; then
        seed_sabnzbd_client || true
    fi
    if [ -n "${SONARR_API_KEY:-}" ] && [ -n "${QBT_WEBUI_PASSWORD:-}" ]; then
        seed_qbittorrent_client || true
    fi
    if [ -n "${SONARR_API_KEY:-}" ] && [ -n "${PROWLARR_API_KEY:-}" ]; then
